# Мой тестовый репозиторий
## Что я сделал
1. Установид git специального сайта https://git-scm.com/install/windows
2. Настроил git
   ```
   git config --global user.name "Dima"
   git config --global user.imail "wsfhskjhf-mail.ru"
   ```
3. Создал репозиторий
   ```
   git inint
   ```
4. Добавил файлы для коммита
   ```
   git add main.py
   ```
5. Создал коммит с коментарием
   ```
   git commit -m "мой первый коммит"
   ```
## Как я опубликовал репозиторий
1. Создал репизиторий в github
2. Связал локальный репозиторий с облачным
   ```
   git remote add origin https://github.com/dimaiksar10-ship-it/git-test.git
   git branch -M main
   git push -u origin main
   ```
## Полезные команды git
`git status` - Проверить текущий статус

`git log` - Посмотреть историю коммитов. Добавить `--oneline` для краткого вывода

`git add -A` - Добавить все файлы в текущий коммит

`git commit -m "мой первый коммит"` - Создать коммит

`git push` - Отправить все изменения в облачный репозиторий (на github)

`git pull` - Скачать все изменения с облачного репозитория в локальный

`git clone` - Клонировать репозиторий с github. Ссылку можно найти на странице репозитория под зелёной кнопкой "Code"

## Работа с ветками
Ветка — это отдельная «параллельная» копия проекта, в которой ты можешь спокойно экспериментировать
`git branch` - посмотреть список веток

`git switch имя_ветки` - переключится на другую ветку

`git switch -c имя_ветки` - создать новою ветку

`git merge имя_ветки` - забрать изменения из ветки "имя_ветки" в текущую ветку

`git branch -d имя_ветки` - посмотреть список веток

## Алгоритм работы с ветками
1. Создать отцельную ветку
2. Внести и закомитить изменения
3. Произвести слияние веток